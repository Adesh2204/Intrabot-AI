...
def filter_hits_by_role(hits, role):
    """
    hits: list of dicts with keys: source, text, distance, roles
    role: string
    returns filtered hits where the role is in the roles list
    """
    return [h for h in hits if role in h.get("roles", "").split(",")]